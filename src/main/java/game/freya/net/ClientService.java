...
public class ClientService {
    ...
    public void openSocket(String host, Integer port, GameController gameController) {
        if (connectionThread != null && connectionThread.isAlive()) {
            throw new GlobalServiceException(ErrorMessages.WRONG_STATE,
                    "Попытка создать поток сокетного подключения к Серверу при уже существующем потоке.");
        }
                ...
                try (Socket client = new Socket(host, port != null ? port : Constants.SERVER_PORT) {
                    {
                        ...
                        setSoTimeout(Constants.SERVER_CONNECTION_AWAIT_TIMEOUT);
                    }
                }) {
